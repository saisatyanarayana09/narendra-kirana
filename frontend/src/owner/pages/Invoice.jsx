    ...
    >
      ...
      <div className="max-w-4xl mx-auto mb-6 flex flex-col sm:flex-row gap-3 justify-between items-center print:hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2.5 w-full sm:w-auto">
          ...
          <button
            onClick={handlePrint}
            className="w-full sm:w-auto justify-center inline-flex items-center bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-emerald-700 shadow-sm transition-colors text-sm cursor-pointer"
          >
      ...
      >
        ...
        <div className={`relative z-10 ${order.status === 'REJECTED' ? 'opacity-70 grayscale-[30%]' : ''}`}>
              ...
              <div className="text-sm text-slate-600 space-y-0.5 leading-relaxed">
                ...
                {settings?.gstin && (
                  <p className="text-xs font-semibold text-slate-700 mt-1">
                    GSTIN: <span className="font-mono text-slate-900 tracking-wide">{settings.gstin}</span>
                  </p>
                )}
                {settings?.fssai_license_number && (
                  <p className="text-xs font-semibold text-slate-700 flex items-center gap-1">
                    <span>FSSAI Lic. No:</span>
                    <span className="font-mono text-slate-900 tracking-wide">{settings.fssai_license_number}</span>
          ...
          {/* Footer & Signatures */}
          <div className="border-t border-slate-300 pt-6 mt-auto print:break-inside-avoid flex flex-col sm:flex-row gap-8 justify-between items-start sm:items-end">
            <div className="text-xs text-slate-600 max-w-md space-y-3">
              {/* Legal & Compliance Badges in Footer */}
              {(settings?.fssai_license_number || settings?.gstin) && (
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2.5 space-y-1 print:bg-transparent print:border-slate-300">
                  {settings?.fssai_license_number && (
                    <div className="flex items-center gap-1.5 font-medium text-slate-800">
                      <BadgeCheck className="w-4 h-4 text-emerald-600 shrink-0" />
                      <span>FSSAI Lic. No:</span>
                      <span className="font-mono font-bold text-slate-900">{settings.fssai_license_number}</span>
                      <span className="text-[10px] uppercase font-bold bg-emerald-100 text-emerald-800 px-1.5 py-0.5 rounded print:border print:border-emerald-500">Verified</span>
                    </div>
                  )}
                  {settings?.gstin && (
                    <div className="flex items-center gap-1.5 font-medium text-slate-800">
              ...
              {/* Terms & Conditions */}
              <div>
                <h4 className="font-bold text-slate-800 mb-1.5 uppercase tracking-widest flex items-center text-[11px]">
                  <BadgeCheck className="w-3.5 h-3.5 mr-1 text-emerald-700" /> Terms & Conditions
                </h4>
                <div className="space-y-1 text-slate-600 leading-relaxed">
                  {(() => {
                    ...
                    return (
                      <>
                        <p className="mb-0.5">1. Please keep this invoice for your records.</p>
                        <p className="mb-0.5">2. Goods sold are non-refundable without valid receipt.</p>
                      </>
                    );
                  })()}
                </div>
                <p className="text-slate-800 font-semibold mt-2">Thank you for shopping with Narendra Kirana!</p>
              </div>
            </div>
            ...
            <div className="text-left sm:text-right w-full sm:w-auto">
              ...
              <p className="text-[10px] text-slate-500 mt-1 uppercase tracking-widest">{settings?.store_name || "Narendra Kirana"}</p>
            </div>
          </div>
