...
function ProductImage({ product, large = false }) {
 ...
 <img src={optimizeImage(product.image)} alt={product.name} className="w-full h-full object-cover mix-blend-multiply transition-transform duration-700 group-hover:scale-110"/>
 </div>
 );
 }
    ...
    <GSAPZoomIn className="mt-4 flex gap-3 overflow-x-auto pb-4 hide-scrollbar" stagger={0.05}>
      {categories.map((category, index) => (
        ...
        <div className="absolute inset-0">
          {category.image ? (
            <img src={optimizeImage(category.image)} alt={category.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500"/>
          ) : (
