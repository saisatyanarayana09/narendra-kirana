    ...
    <div>
      <div className="mb-6 border-b border-slate-200 pb-4">
        ...
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Favorites</h2>
      ...
      {favorites.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="text-2xl font-black text-slate-900 mb-2">No favorites yet</h3>
          <p className="text-slate-500 mb-8 max-w-md mx-auto">Keep track of the products you love by clicking the heart icon on any product.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {favorites.map(fav => (
            <div key={fav.id} className="group flex items-center gap-4 bg-white rounded-2xl p-4 shadow-sm border border-slate-200 hover:border-primary-300 hover:shadow-md transition-all">
              ...
              <div className="flex-1 min-w-0 py-1">
                <Link to={`/product/${fav.product_details?.id}`} className="font-extrabold text-slate-900 truncate block hover:text-primary-600 transition-colors text-base mb-0.5">{fav.product_details?.name}</Link>
                <p className="text-xs font-medium text-slate-500 mb-2">{fav.product_details?.unit}</p>
                <div className="flex items-center justify-between">
                  <p className="font-black text-slate-900">₹{fav.product_details?.regular_price}</p>
                </div>
              </div>
              <button onClick={() => toggleFavorite(fav.product_details?.id)} className="shrink-0 p-2.5 text-rose-500 bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors self-start" title="Remove from favorites">
