...
old_func = """function ProductImage({ product, large = false }) {

 if (product.image) {

 return (

 <div className={w-full flex items-center justify-center relative overflow-hidden bg-gradient-to-b from-transparent to-slate-50/50 }>
 ...
 return <div className={grid w-full place-items-center bg-slate-100 text-3xl font-bold text-slate-300 }>{product.name?.charAt(0)?.toUpperCase()}</div>
       ...
       return (
          <div className="flex flex-col h-full w-full">
            ...
            <div className="flex gap-3 p-3 overflow-x-auto bg-slate-50 border-t border-slate-100">
               {images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImage(i)} className={lex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden shadow-sm transition-all }>
