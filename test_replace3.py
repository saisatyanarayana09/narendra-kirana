...
old_sku_field = '''<label className="block text-sm font-bold text-slate-700 mb-1.5">SKU / Barcode</label>
...
new_sku_field = '''<label className="block text-sm font-bold text-slate-700 mb-1.5 flex justify-between items-center">
...
if old_sku_field in content:
    content = content.replace(old_sku_field, new_sku_field)
    print("Success")
else:
    print("Still failed")
