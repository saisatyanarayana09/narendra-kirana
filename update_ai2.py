...
# 1. Groq Prompt
content = re.sub(
    r'4\. Return ONLY the description text, no quotes or intro\.',
    r'4. ABSOLUTELY NO MARKDOWN. Do not use asterisks (**), bolding, bullet points, or hashes.\n            5. Return pure plain text only. No quotes, no intro, no emojis.',
    content
)

# 2. Analyze Image Signature and Logic
old_def = '''    @staticmethod
    def analyze_product_image(image_bytes, mime_type='image/jpeg'):
        try:
...
if 'def analyze_product_image(image_bytes, mime_type=\'image/jpeg\'):' in content:
    ...
    print("Replaced analyze def")
else:
    print("Could not find analyze def")
