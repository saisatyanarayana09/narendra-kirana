...
# I will use Regex to safely replace the SKU field div.
pattern = re.compile(r'   <div>\s*<label className="block text-sm font-bold text-slate-700 mb-1\.5">SKU / Barcode</label>\s*<input type="text" value=\{formData\.sku\}[^>]*/>\s*</div>', re.DOTALL)
...
if pattern.search(content):
    content = pattern.sub(new_sku, content)
    print("Replaced SKU field with scanner button")
else:
    print("Failed to find SKU field")
