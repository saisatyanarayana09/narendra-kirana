    ...
    try {
      ...
      const errMsg =
        typeof errData === "string"
          ? errData
          : errData?.error || errData?.detail || "Failed to send OTP code.";
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      if (res.data?.valid) {
        ...
        setOtpError(
          res.data?.error || "Invalid verification code. Please try again.",
        );
      }
    } catch (err: any) {
      const errData = err.response?.data;
    ...
    try {
      if (mode === "otp") {
        ...
        if (!uid || !token) {
          Alert.alert(
            "Invalid Link",
            "This reset link is missing security parameters. Please switch to 6-Digit OTP.",
          );
          setIsLoading(false);
          return;
        }
      ...
      const errMsg =
        typeof errData === "string"
          ...
          : errData?.error ||
            errData?.detail ||
            errData?.message ||
            "Failed to reset password. The link or OTP may have expired.";
        ...
        >
          ...
          <View style={styles.header}>
            ...
            >
              <Feather
                name={
                  mode === "otp" && otpStep === "verify" ? "shield" : "lock"
                }
                size={28}
          ...
          >
            ...
            >
              <Feather
                name="link"
                size={14}
                color={mode === "link" ? colors.primary : colors.textSecondary}
              />
              <Text
                style={[
                  styles.tabText,
                  {
                    color:
                      mode === "link" ? colors.primary : colors.textSecondary,
                    fontWeight: mode === "link" ? "700" : "600",
                  },
                ]}
              >
                Email Link
            ...
            >
              ...
              <Text
                style={[
                  ...
                  {
                    color:
                      mode === "otp" ? colors.primary : colors.textSecondary,
                    fontWeight: mode === "otp" ? "700" : "600",
                  },
                ]}
              >
                6-Digit OTP
                ...
                {mode === "link" && (!uid || !token) && (
                  ...
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      >
                        Open the reset link from your email inbox to proceed, or
                        switch to 6-Digit OTP.
                      </Text>
                      <View
