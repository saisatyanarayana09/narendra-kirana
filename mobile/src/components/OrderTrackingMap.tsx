...
export function OrderTrackingMap({
  ...
  const hasRiderPosition = riderLat !== null && riderLng !== null;
  const originLat = hasRiderPosition ? riderLat : storeLat;
  const originLng = hasRiderPosition ? riderLng : storeLng;

  // Do NOT show store pin if rider is on the road to customer
  const showStorePin = !hasRiderPosition && !isOutForDelivery;
  ...
  const trackingHtml = useMemo(() => {
      ...
      <html>
        <head>
          ...
          <style>
            * { box-sizing: border-box; }
            html, body, #map { margin: 0; padding: 0; width: 100%; height: 100%; background: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; }
            
            /* Pin Styles */
            .store-pin {
              width: 32px; height: 32px; background: #064E3B; border: 2.5px solid #ffffff;
        ...
        <body>
          ...
          <div class="eta-pill" id="eta-pill">
            ...
            <span id="eta-text">${hasRiderPosition ? 'Connecting live rider route...' : 'Delivery Route'}</span>
          </div>
    ...
    `;
  }, [storeLat, storeLng, custLat, custLng, riderLat, riderLng, originLat, originLng, showStorePin, hasRiderPosition, riderName, custAddress]);

  const handleOpenMaps = () => {
    const destCoords = `${custLat},${custLng}`;
